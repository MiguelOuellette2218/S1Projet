...
void cheatCodeProc(vector* gauche, vector* droite)
{
    if(bumper() == 3) // Si bumper arrière
    {
        ...
        {
            delay(20);
            tmpg = vector_push(gauche, ENCODER_Read(LEFT));
            tmpd = vector_push(droite, ENCODER_Read(RIGHT));
            Serial.println(tmpd);
            stop = bumper();
        }
        
    }
    for(ul i=0; i < gauche->size; i++)
    {
        Serial.println(gauche->data[i]);
    }
    //vector_clean(gauche);
    //vector_clean(droite);
}
...
void cheatProc(int32_t* gauche, int32_t* droite)
{
    ...
    if(bumper() == 3) // Si bumper arrière
    {
        ...
        {
            delay(20);
            gauche[count] = ENCODER_Read(LEFT);
            droite[count] = ENCODER_Read(RIGHT);
            stop = bumper();
            count++;
        }
